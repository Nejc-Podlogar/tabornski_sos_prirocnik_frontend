...
import 'package:tabornski_sos_prirocnik_frontend/views/orientation/topo_characters_learning_selector.dart';
...
import '../views/orientation/orientation_cards.dart';
  ...
  static GoRouter router = GoRouter(
    ...
    routes: [
                  ...
                  GoRoute(
                      ...
                      pageBuilder: (context, state) {
                        ...
                        if (state.extra != null) {
                          ...
                          if (extra['params']
                          is MorseCodeLearningParams) {
                            params = extra['params']
                            as MorseCodeLearningParams;
                          }
                        }

                        if (params == null) {
                          throw Exception(
                              'MorseCodeLearningParams are not provided');
                        }

                        return MaterialPage(
                            child: LearningScreen(params: params));
                      })
                ])
          ]),
      GoRoute(
      ...
      GoRoute(
          ...
          routes: <GoRoute>[
            GoRoute(
                path: RoutePaths.orientationLearningSelector,
                name: RouteNames.orientationLearningSelector,
                pageBuilder: (context, state) {
                  return const MaterialPage(
                      child: TopoCharactersLearning());
                }),
            GoRoute(
                name: RouteNames.orientationLearning,
                path: RoutePaths.orientationLearning,
                pageBuilder: (context, state) {
                  OrientationExerciseParams? params;

                  if (state.extra != null) {
                    var extra = state.extra as Map<String, dynamic>;
                    if (extra['params'] is OrientationExerciseParams) {
                      params = extra['params'] as OrientationExerciseParams;
                    }
                  }

                  if(params == null) {
                    throw Exception('OrientationExerciseParams are not provided');
                  }

                  return MaterialPage(child: OrientationScreenCards(params: params));
                })
          ]),
      GoRoute(
    ...
    redirect: (BuildContext context, state) async {
      final bool showWelcome = await SharedPrefs().isFirstTime();

      if ((showWelcome && state.matchedLocation == RoutePaths.root) || (!showWelcome && state.matchedLocation == RoutePaths.home)) {
        return null;
      }else if (!showWelcome && state.matchedLocation == RoutePaths.root) {
